...
#'             NJ Higham (1988, Linear Algebra Appl. 103:103-118).
#' @examples \dontrun{
...
dpout<-function(lambda, plotting=TRUE,...)
{
  ...
  if (plotting==TRUE) {  
    ...
    #plot(d, lambda, type="l")
    #lines(bstck)
    plot(thmax[ijump],d[ijump], main="D-Plot", xlab="Theta", ylab="d(Theta) - Harmonics", type="S")
    #plot( thmax[ijump], d[ijump], main="D-Plot", xlab="Theta", ylab="d(Theta)", type="S")
...
#
# lamb<-seq(0,1,0.1)
# dpout<-function(lamb, plotting=TRUE) 
#
#
